...
在现有浏览器窗口中导航。工具会复用匹配或明确指定的标签页；否则在现有窗口中新建标签页。此工具绝不会创建新的浏览器窗口。

**参数**：

- `url` (字符串，可选)：要导航到的 URL（`refresh=true` 时可省略）
- `tabId` (数字，可选)：要导航或刷新的现有标签页 ID
- `windowId` (数字，可选)：用于查找或创建标签页的现有窗口 ID
- `background` (布尔值，可选)：不激活标签页或聚焦窗口（默认：false）
